...
if exist wssm-config.cfg (
    for /f "skip=4" %%l in (wssm-config.cfg) do if not defined SsdSlowMark set SsdSlowMark=%%l
    for /f "skip=7" %%l in (wssm-config.cfg) do if not defined Java set Java=%%l
    for /f "skip=10" %%l in (wssm-config.cfg) do if not defined FileCount set FileCount=%%l
    for /f "skip=13" %%l in (wssm-config.cfg) do if not defined TestType set TestType=%%l
    for /f "skip=16" %%l in (wssm-config.cfg) do if not defined FileSize set FileSize=%%l
    for /f "skip=19" %%l in (wssm-config.cfg) do if not defined BlockSize set BlockSize=%%l
    for /f "skip=22" %%l in (wssm-config.cfg) do if not defined DumpFolder set DumpFolder=%%l
    for /f "skip=25" %%l in (wssm-config.cfg) do if not defined ResultsFolder set ResultsFolder=%%l
    for /f "skip=28" %%l in (wssm-config.cfg) do if not defined ImageWidth set ImageWidth=%%l
    for /f "skip=31" %%l in (wssm-config.cfg) do if not defined ImageHeight set ImageHeight=%%l
    for /f "skip=34" %%l in (wssm-config.cfg) do if not defined ImagePadding set ImagePadding=%%l
) else (
    echo Configuration file "wssm-config.cfg" not found. Download it again in "https://github.com/KaioHSG/WinSsdSlowMark/releases/latest".
    pause > nul
    exit
)
